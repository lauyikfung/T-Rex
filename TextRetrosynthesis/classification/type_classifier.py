...
class TypeClassifier(pl.LightningModule):
    def __init__(self, data_root, bert_name, num_labels = 10, 
        ...
        if self.graph_model_dir is not None:
            self.has_load = False
            self.hidden_size += 1536 # self.graph_model.output_dim
        ...
        self.data = {"train": TypeClassifierDataset(data_root, 'train'), "valid": TypeClassifierDataset(data_root, 'valid'), "test": TypeClassifierDataset(data_root, 'test')}
    ...
    def forward(self, text1_inputs, mol_list):
    ...
    def train_dataloader(self):
        return torch.utils.data.DataLoader(
            self.data['train'],
            collate_fn=TypeClassifierCollateFn(tokenizer_name=self.hparams.bert_name),
...
class TypeClassifierDataset(Dataset):
    ...
    def _load_data(self, data_root):
        with open(data_root, "r") as f:
            ...
            for row in tqdm(datasets):
                data.append({"text": row[-1], "label": int(row[2]) - 1, "smiles": row[4]})
        return data
