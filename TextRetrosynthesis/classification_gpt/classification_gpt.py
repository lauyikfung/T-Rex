...
class ReactionClassification_GPT(pl.LightningModule):
    def __init__(self, data_root, bert_name, num_labels, dropout_rate=0.1, lr=1e-5, batch_size=128, max_epochs=1000, num_workers=4, bert_layer_train_num = 1, alpha = 0.0, beta = 0.0, graph_model_dir = None, fold=1):
        super().__init__()
    ...
    def compute_top1_acc(self, data_reactant, tokenizer, device, bsz=3):
        ...
        for i in tqdm(range(len(data_reactant) // bsz)):
            ...
            text1_list = [row[-1] for row in data_reactant[i * bsz:i * bsz + bsz]]
            text_list = [text1 for text1 in text1_list]
    ...
    def _load_data(self, data_root):
        with open(data_root, "r") as f:
            ...
            for row in tqdm(datasets):
                data.append({"text": row[-1], "label": (row[3] == "True") + 0, "smiles": row[2]})
        return data
