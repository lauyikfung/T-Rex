...
class EngineText(core.Configurable):
    """
    ...
    def train(self, num_epoch=1, batch_per_epoch=None):
        ...
        for epoch in self.meter(num_epoch):
            ...
            for batch_id, batch in enumerate(islice(dataloader, batch_per_epoch)):
                text_batch = {}
                for t_k in self.train_set.dataset.text_fields:
    ...
    @torch.no_grad()
    def evaluate_3(self, split, log=True):
        """
        Evaluate the model.
        ...
        for batch in tqdm(dataloader):
            ...
            pred, target = model.predict_3(batch, only_3=True), model.target(batch)
            preds.append(pred)
            targets.append(target)
