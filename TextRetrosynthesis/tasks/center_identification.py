...
class CenterIdentificationText(tasks.Task, core.Configurable):
    """
    ...
    def __init__(self, model, feature=("reaction", "graph", "atom", "bond"), num_mlp_layer=2,
                 ...
                 max_len=512,
                 alpha=0.00):
        super(CenterIdentificationText, self).__init__()
        self.model = model
        ...
        self.PLM_d = PLM_d
        # self.text_fields = ['MolT5', 'ChatGPT']
        self.text_fields = ['MolT5', 'ChatGPT']
    ...
    def get_embeddings(self, model, tokenized_texts: List[List[str]], tokenizer, device):
        embeddings = []
        for token_chunks in tokenized_texts:
            input_ids = [tokenizer.encode(chunk, add_special_tokens=True, padding='max_length', max_length=self.max_len+2) \
                         for chunk in token_chunks]
            input_ids = torch.tensor(input_ids).to(device)
            outputs = model(input_ids)
            chunk_embeddings_mean = outputs.last_hidden_state[:, -1, :].mean(dim=0)
            #chunk_embeddings_max = outputs.last_hidden_state[:, -1, :].max(dim=0)[0]
            #chunk_embeddings = torch.cat([chunk_embeddings_mean, chunk_embeddings_max])
            #chunk_embeddings = torch.functional.F.normalize(chunk_embeddings, dim=0)
            embeddings.append(chunk_embeddings_mean)
    ...
    def encode_text(self, batch):
        ...
        embeddings = self.get_embeddings(self.PLM, \
                                         self.tokenize_and_cut(self.tokenizer, text, self.max_len), 
