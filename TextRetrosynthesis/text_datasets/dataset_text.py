...
class USPTOText50k(data.ReactionDataset):
    ...
    def __init__(self, path, file_name, as_synthon=False, verbose=1, **kwargs):
        ...
        self.load_csv(path + file_name, smiles_field="rxn_smiles", target_fields=self.target_fields + self.text_fields, verbose=verbose,
                      **kwargs)
        if as_synthon:
        ...
        for i in indexes:
            ...
            # reactants, products = [reactant], [product]
            if not reactants:
                invalid += 1
                continue
    ...
    # define a function to canocalize the SMILES string
    ...
    def split_new(self, ratios=(0.8, 0.1, 0.1), nums=None):
        if nums is not None:
            sample_id_list = self.targets["sample id"]
            indexes = [[] for _ in range(len(nums))]
            index_list = [i for i in range(len(sample_id_list))]
            for i in range(len(nums) - 1):
                indexes[i] = [j for j in index_list if sample_id_list[j] >= sum(nums[:i]) and sample_id_list[j] < sum(nums[:i+1])]
                random.shuffle(indexes[i])
            indexes[len(nums) - 1] = [j for j in index_list if sample_id_list[j] >= sum(nums[:len(nums) - 1])]
            random.shuffle(indexes[len(nums) - 1])
    ...
    def reactant_based_split(self, ratios=(0.8, 0.1, 0.1)):
        react2index = defaultdict(list)
        react2sample = defaultdict(list)
        index2sample = dict()
        for i in range(len(self)):
            ...
            react2index[reaction].append(i)
            react2sample[reaction].append(sample_id)
            index2sample[i] = sample_id

        # get the reactants for each sample
        sample2reactants = list()
        for i in range(len(self.smiles_list)):
            reactants = self.smiles_list[i].split(">>")[0].split(".")
            # canonicalize the reactants
            reactants = [self._canonicalize(reactant) for reactant in reactants]
            sample2reactants.append(reactants)
        ...
        results = [torch_data.Subset(self, index) for index in indexes]
        eval_reactants = set()
        for i in [1, 2]:
            for ii in range(len(results[i])):
                sample_id = results[i][ii]['sample id']
                reactants = sample2reactants[sample_id]
                for r in reactants:
                    eval_reactants.add(r)
        drop_indices = []

        for i in range(len(results[0])):
            sample_id = results[0][i]['sample id']
            reactants = sample2reactants[sample_id]
            for r in reactants:
                if r in eval_reactants:
                    drop_indices.append(results[0].indices[i])
                    break
        results[0] = torch_data.Subset(self, [i for i in results[0].indices if i not in drop_indices])
        print("Drop {} samples from training set.".format(len(drop_indices)))
        return results
